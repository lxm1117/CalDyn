...
c
	ir=34
	id=39
	open(file='dif0504cr.dat',unit=ir,status='old')
c
...
c                  write(6,*) ' add ',ksite,nsite,iTcamf,rr
                   ...
                   if(iss.eq.isu+1) icamk(ihh,1,2)=1
                       write(6,*) ' somewhere to bound 0->b ',x,pr(1),rr
                   iTcamf=iTcamf-1
                   iTcamb=iTcamb+1
...
c           endif
                   iTcamf=iTcamf+1
                   iTcamb=iTcamb-1
                       write(6,*) ' bound-to somewhere b->0 ',x,pr(1),rr
                   icamb(ihh)=icamb(ihh)-1
                   icamf(ihh)=icamf(ihh)+1
...
c	 zk(5)=ak2pp*y(7*n+14)*camkph/(ppkm+camkph)
c	 zk(7)=ak2pp*y(7*n+14)*camkph/(ppkm+camkph)
c	 zk(8)=ak2pp*y(7*n+14)*camkph/(ppkm+camkph)
c	 zk(10)=ak2pp*y(7*n+14)*camkph/(ppkm+camkph)
...
c
 ...
 10    format(a40)
	  ...
	  read(id,*) ic,al(i),d(i)
 200   continue
       write(6,*) ' enter last spine head compartment '
       read(id,*) lsh
...
c                      write(6,*) ' testing a b ',x,rr,pr(3)
		    ...
		    idtrap = idtrap + 1
                    ckb=ckb-amol
                    write(6,*)  ' bound-to-trapped ', x,rr,pr(3)
		 endif	
...
c    c                             ' b->t2 ',x,pr(2),pr(2)+pr(3),rr
		    ...
		    idtrap = idtrap + 1
                    ckb=ckb-amol
                    write(6,*)  ' bound-to-trapped2 ', x,pr(2),pr(2)+pr(3),rr
	         endif
